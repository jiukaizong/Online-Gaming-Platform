...
Game.prototype.place = function(column) {
    if(window.user_round == that.round){
        $("#turn").innerText="Your turn";
    }
    else{
        $("#turn").innerText = "Opponent's turn";
    }
    console.log(`这是${that.round}的回合，它的column是：${column}`)
    // If not finished
...
Game.prototype.updateStatus = function() {
    ...
    if (that.board.score() ==  -that.score) {
        ...
        alert(game_info.first_user.name+ " have won!");
    }
    ...
    if (that.board.score() == that.score) {
        ...
        alert(game_info.second_user.name +" have lost!");
    }
    ...
    if (that.status == 0) {
        ...
        html.innerHTML = game_info.first_user.name+" won";
    } else if (that.status == 2) {
        html.className = "status-won";
        html.innerHTML = game_info.second_user.name +"won";
    } else {
