...
Game.prototype.act = function(e) {
    ...
    if (!($('#coin').is(":animated"))) {
        if (that.round == window.user_round) {
            that.place(element.cellIndex);
            gameSocket.emit("send_move",{move:element.cellIndex,game_id:window.gameId});
        }
    }
}
...
Game.prototype.updateStatus = function() {
    ...
    // Human won
    if (that.board.score() == (user_round? that.score: -that.score)) {
        that.status = 1;
        that.markWin();
        GameOver(0);
        alert("You have won!");
